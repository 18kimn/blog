...
print_skills <- function(cv){
  map(unique(cv$skills$category), function(cat){
    dta <- cv$skills %>% filter(category == cat) %>%
      pull(skill)
    cat_format <- paste0("<b>",cat, "</b>")
    paste0(
      paste0(c(cat_format,dta), collapse = "<br>"),
           "<br>")
  } ) %>%
    unlist() %>%
    paste0(collapse = "<br>") %>%
