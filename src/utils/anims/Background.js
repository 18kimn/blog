...
const runBackgroundMap = () => {
  ...
  const loading = d3.select('#gatsby-focus-wrapper')
    .append('div')
    .style('position', 'absolute')
    .style('bottom', 0)
    .style('left', 0)
  ...
  const loadingDots = () => {
    loading.transition()
      ...
      .tween('text', () => function(t) {
        ...
        const expr = `loading background animation${'.'.repeat(Math.round(t * 4))}`
  ...
  d3.json('/nhv_blocks.json').then((nhvBlocks) => {
    loading.remove()
